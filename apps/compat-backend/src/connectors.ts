...
// Granola official API (Bearer grn_ key). The key is generated by the user in the Granola
// desktop app (Settings → Connectors → API keys); the console's GranolaConnectModal posts it as
// metadata.apiKey. Transcripts are large/noisy, so only the AI summary is ingested by default.
...
// Quick key probe: distinguishes a bad key (reject at connect) from a transient error (allow).
async function granolaCheckKey(apiKey: string): Promise<"ok" | "invalid" | "unknown"> {
	try {
		const res = await fetch(`${GRANOLA_API}/notes?limit=1`, {
...
async function granolaListNotes(
	apiKey: string,
	opts: { cursor?: string; createdAfter?: string },
): Promise<{ notes: GranolaNote[]; cursor?: string; hasMore: boolean }> {
	const p = new URLSearchParams({ limit: "100" })
	if (opts.cursor) p.set("cursor", opts.cursor)
	if (opts.createdAfter) p.set("created_after", opts.createdAfter)
	if (GRANOLA_INCLUDE_TRANSCRIPT) p.set("include", "transcript")
	const res = await fetch(`${GRANOLA_API}/notes?${p.toString()}`, {
		headers: { authorization: `Bearer ${apiKey}` },
	})
	if (res.status === 429) {
		await sleep(2000)
		return granolaListNotes(apiKey, opts)
	}
	if (!res.ok) throw new Error(`granola list failed: ${res.status} ${await res.text()}`)
...
function granolaNoteToContent(n: GranolaNote): string {
	...
	if (GRANOLA_INCLUDE_TRANSCRIPT && n.transcript) {
		const t = Array.isArray(n.transcript)
			? (n.transcript as Array<{ text?: string }>).map((u) => u.text).filter(Boolean).join("\n")
...
// Pulls the user's Granola notes into the lite store (mirrors runSync). Incremental via the
// created_after filter (last successful sync time is kept in the connection metadata).
async function runGranolaSync(connId: string, trigger: "manual" | "event" | "cron") {
	const r = db.query("SELECT * FROM sm_connection WHERE id=?").get(connId) as ConnRow | null
	if (!r || r.status !== "connected" || !r.access_token) return
	const runId = rid("run")
	db.query(
		"INSERT INTO sm_sync_run (id, connection_id, status, trigger_type, started_at, items_processed, items_failed) VALUES (?,?,?,?,?,0,0)",
	).run(runId, connId, "running", trigger, nowIso())
	...
	let failed = 0
	try {
		const tags = r.container_tags ? (JSON.parse(r.container_tags) as string[]) : ["sm_org_shared"]
		const meta = r.metadata ? (JSON.parse(r.metadata) as Record<string, unknown>) : {}
		const createdAfter = typeof meta.lastSyncedAt === "string" ? meta.lastSyncedAt : undefined
		const startedAt = nowIso()
		let cursor: string | undefined
		let logged = false
		for (let page = 0; page < 50; page++) {
			// safety cap: 50 pages × 100 notes
			const { notes, cursor: next, hasMore } = await granolaListNotes(r.access_token, {
				cursor,
				createdAfter,
			})
			if (!logged && notes[0]) {
		...
		db.query("UPDATE sm_connection SET metadata=? WHERE id=?").run(
			JSON.stringify({ ...meta, lastSyncedAt: startedAt }),
			connId,
		)
		db.query(
			"UPDATE sm_sync_run SET status=?, completed_at=?, items_processed=?, items_failed=? WHERE id=?",
...
connectors.post("/:provider/import", async (c) => {
	...
	for (const r of rows) {
		...
		const doSync = provider === "granola" ? runGranolaSync : runSync
		doSync(r.id, "manual").catch(() => {})
	}
...
connectors.post("/:provider", async (c) => {
	...
	if (provider === "granola") {
		const body = (await c.req.json().catch(() => ({}))) as {
			...
			metadata?: { apiKey?: string } & Record<string, unknown>
		}
		const apiKey = body.metadata?.apiKey?.trim()
		if (!apiKey) return jsonError("missing Granola API key (metadata.apiKey)", 400)
		if ((await granolaCheckKey(apiKey)) === "invalid") return jsonError("invalid Granola API key", 400)
		const tags = body.containerTags?.length ? body.containerTags : ["sm_org_shared"]
		// Never persist the key in metadata — toConnectionResponse returns metadata to the client.
		const meta: Record<string, unknown> = { ...(body.metadata ?? {}) }
		delete meta.apiKey
		const id = rid("conn")
		db.query(
			`INSERT INTO sm_connection (id, provider, user_id, status, container_tags, access_token, metadata, created_at)
     VALUES (?,?,?,?,?,?,?,?)`,
		).run(
			id,
			"granola",
		...
		runGranolaSync(id, "event").catch(() => {})
		const row = db.query("SELECT * FROM sm_connection WHERE id=?").get(id) as ConnRow
